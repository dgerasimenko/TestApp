...
public class Transformer {
    ...
    public void extractAndTransform(AtomicInteger chunkWithDuplicates) {
        ...
        if (CollectionUtils.isEmpty(currentProcessingState)) {
            ...
            for(TaskInfo taskInfo : currentProcessingState) {
                ...
                executor.submit(new ScrapperTask(flightDao, taskInfoDao, chunk, taskInfo, totalHandledRecords, chunkWithDuplicates, taskTime));
            }
       ...
       //     while (tmpChunkSize > 0) {
                ...
                executor.submit(new ScrapperTask(flightDao, taskInfoDao, chunk, null, totalHandledRecords, chunkWithDuplicates, taskTime));
    ...
    public void extractAndTransform1(TaskInfo taskInfo, Long prevRecordId, AtomicInteger counter) {
        ...
        try {
            ...
            while (tmpChunkSize > 0) {
                Long startIndex = chunk.get(tmpChunkSize - 1).getId();
                executor.submit(new ScrapperTask(flightDao, taskInfoDao, chunk, taskInfo, totalHandledRecords, counter, taskTime));
                chunk = flightDao.getNextChunk(startIndex, chunkSize);
                taskInfo = null;
                tmpChunkSize = chunk.size();
